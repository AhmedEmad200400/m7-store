...
function resetAllDefaults() {
    if (!confirm("Are you sure you want to completely clear the entire catalog? This will delete ALL products from the live website.")) {
        return;
    }
    
    window.db.ref('products').set(null).then(() => {
        alert("Catalog successfully cleared! You are now starting on a clean slate.");
        if (typeof renderCatalogTable === 'function') renderCatalogTable();
    }).catch(err => {
